...
import {Text,StyleSheet , View, TouchableOpacity,TextInput, Button, Dimensions} from 'react-native';
import AsyncStorage from '@react-native-async-storage/async-storage';

export default function Hra2 (){
    ...
    const Pocitanie = async() => {
        ...
        if (Odpoved == Vysledok){
            ...
            znamienko = Znamienka[(Math.floor(Math.random() * Znamienka.length) + 0)];
            if (znamienko == "+") {
                cislo2 = (Math.floor(Math.random()*(rozsah - cislo1)) +0);
                setVysledok(cislo1 + cislo2);
            }
            if (znamienko == "-"){
                cislo2 = (Math.floor(Math.random()*(cislo1)) +0);
                setVysledok(cislo1 - cislo2);
            }
            setTlacidlo(<Text>{cislo1} {znamienko} {cislo2} = </Text>);
        }
